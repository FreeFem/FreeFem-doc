...
```
<!--- > -->
As illustrated in figure 1.2, we can see the isovalue of $u$ by using Freefem++ `:::freefem plot` command (see line 29
above).

Figure 1.1 - Mesh Th by `build(C(50))`  |  Figure 1.2 - isovalue by `plot(u)`
:-------------------------:|:-------------------------:
